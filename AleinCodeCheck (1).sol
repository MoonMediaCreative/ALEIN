...
function queue(bytes calldata _proposal) external onlyOwner {  // Use the onlyOwner modifier
        bytes32 proposalHash = keccak256(_proposal);
...
contract AleinToken is ERC20, Ownable, ERC20Burnable {
    ...
    uint256 private totalFees = liquidityFee + marketingFee + projectFundsFee;
    ...
    constructor(
        ...
        address _timelock
    ) ERC20("Alein Token", "ALEIN") Ownable(msg.sender) { // Pass msg.sender to Ownable constructor
        _mint(_msgSender(), _initialSupply); // Use msg.sender for minting
        marketingWallet = _marketingWallet;
        projectFundsWallet = _projectFundsWallet;

        // Initialize the path array with 2 elements
        path = new address[](2);
        path[0] = address(this); // This contract's address
        path[1] = pancakeRouter.WBNB(); // WBNB address from PancakeSwap
